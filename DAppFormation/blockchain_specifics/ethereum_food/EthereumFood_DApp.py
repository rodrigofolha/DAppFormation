    ...
    def startup(dapp_handler):
        ...
        for client, _ in enumerate(client_config['priv_ips']):
            if client == 0:
                ...
                #stdin, stdout, stderr = ssh_clients[0].exec_command(f"cat /home/ubuntu/setup/build/contracts/ChainContract.json | grep address | sed -e 's/      \"address\": \"//g' | sed -e 's/\",//g'")
                #addresses = stdout.readlines()[0].replace("\n", "")
                #stdin, stdout, stderr = ssh_clients[0].exec_command(f"cat /home/ubuntu/setup/build/contracts/ChainContract.json | grep transactionHash | sed -e 's/      \"transactionHash\": \"//g' | sed -e 's/\"//g'")
                #hashes = stdout.readlines()[0].replace("\n", "")
                ...
                #stdin, stdout, stderr = ssh_clients[client].exec_command(f"cat /home/ubuntu/setup/build/contracts/ChainContract.json | grep address | sed -e 's/      \"address\": \"//g' | sed -e 's/\",//g'")
                #addresses1 = stdout.readlines()[0].replace("\n", "")
                #ssh_clients[client].exec_command(f"sed -i 's/{addresses1}/{addresses}/g' /home/ubuntu/setup/build/contracts/ChainContract.json")
                #stdout.readlines()
                ...
                # logger.debug(stdout.readlines())
                # logger.debug(stderr.readlines())
                #stdin, stdout, stderr = ssh_clients[client].exec_command(f"cat /home/ubuntu/setup/build/contracts/ChainContract.json | grep transactionHash | sed -e 's/      \"transactionHash\": \"//g' | sed -e 's/\"//g'")
                #hashes1 = stdout.readlines()[0].replace("\n", "")
                #ssh_clients[client].exec_command(f"sed -i 's/{hashes1}/{hashes}/g' /home/ubuntu/setup/build/contracts/ChainContract.json")
                #stdout.readlines()
        ...
        if len(ssh_clients) > 1:
            ...
            #stdin, stdout, stderr = ssh_clients[1].exec_command("(cd /home/ubuntu/setup && . ~/.profile && node test.js | grep -e getIndex)")
            stdout.readlines()
            # logger.debug("".join(stdout.readlines()))
            # logger.debug("".join(stderr.readlines()))
